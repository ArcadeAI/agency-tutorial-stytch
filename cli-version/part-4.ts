...
const gmailAgent = new Agent({
    ...
    instructions:
        ...
        ${gmailTools.map(tool => `- ${tool.name}`).join("\n")}
        `,
    tools: gmailTools,
});
...
const slackAgent = new Agent({
    ...
    instructions:
        ...
        and you MUST handoff to the triage agent if you need to do something else.
        You can use the following tools to help you:
        ${slackTools.map(tool => `- ${tool.name}`).join("\n")}
        `,
    tools: slackTools,
});
...
async function main() {
    ...
    while (true) {
        ...
        let response = await run(triageAgent, history, { maxTurns: 10 });

        history = response.history;
        display(history, previousHistoryLength);
        previousHistoryLength = history.length;

        while (response.interruptions?.length) {
            console.log(
                chalk.red(
            ...
            response = await run(triageAgent, state, { maxTurns: 10 });
